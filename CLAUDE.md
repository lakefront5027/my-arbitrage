...
| 前置条件 | 校验方式 | 失败处理 |
|---------|---------|---------|
| `navLag`（nav_date 距 benchDate **交易日数**）≤ 3 | `tradingDayLag(navDate, benchDate) ≤ 3` OR 开启链式补偿 | `navLag ≥ 2` 时开启 T-2 链式估值 |
| 时序单向性：`benchDate >= navDate` | `resolveNavBasis()` 内部断言 | 违反则 `navBasis.type = 'INVALID'`，nav = null |
| T-2 链式估值额外前提 | `estNavYesterday != null` AND `syncLagDays ≤ 1` | 不满足则回退至 `officialNav` |
| `syncLagDays`（JSON 同步距 benchDate 交易日数）≤ 1 | `tradingDayLag(syncDateBj, benchDate) ≤ 1` | `syncLagDays ≥ 2` → 禁用链式补偿 |
...
| syncLagDays > 1（JSON 超过1交易日未同步）| `Unavailable`（链式禁用） | — |
...
if (navBasis.type === 'INVALID') { nav = null; /* 时序倒置，拒绝计算 */ }
if (syncLagDays > 1) { useChained = false; /* JSON 超过1交易日未同步 */ }
```
...
触发条件（三重前置锁，缺一不可）：
```
useChained = estNavYesterday != null
