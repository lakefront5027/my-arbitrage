...
| `.github/scripts/test_pdf_holdings.py` | 本地测试脚本：用巨潮 cninfo API 查季报 PDF → pdfplumber 提取持仓（比 DeepSeek 搜索更可靠） |
...
| 链式锚点指数日期 | _(附属于锚点)_ | `est_nav_index_date`（锚点计算消费的 bench 指数交易日） | _(同上)_ |
...
est_nav_date       = T-1 交易日日期（bench 数据的实际交易日，来自 date_map）
est_nav_index_date = T-1 bench 指数交易日（与 est_nav_date 通常相同；历史公证人字段）
...
- `est_nav_index_date` = 计算 `est_nav_yesterday` 时消费的 bench 指数交易日（历史公证人）；与 `est_nav_date` 通常相同，但在 Worker 侧作为独立字段读取，用于幂等锁
- `update_drift()` 内部计算的 `est_nav` 仅用于 drift 对账（`prev_nav × bench_chg`），**不写入 est_nav_yesterday**
- bench 数据不可用时：清除旧 `est_nav_yesterday` / `est_nav_date` / `est_nav_index_date`，防止 Worker 误用过期锚点
- `sinaAG0` 在 Action 的 bench 抓取中通过 `_TQ_ALIASES: sinaAG0 → nf_AG0` 映射至腾讯接口

**T-2 双重计数断路器（US 基金专项）**：

问题背景：当 Yahoo 不可达时（`marketContext.usDate = null`），`benchDate` 降级为 `aShareDate`。此时 Tencent 行情中的 T-1 美股价格与 Action 写入 `est_nav_yesterday` 时消费的 bench 指数属于**同一个美股交易日**，导致同一收益率被乘入两次。

断路器逻辑：
```js
// realtimeIndexDate = 当前实时抓取能覆盖的最新美股指数日期
const realtimeIndexDate = marketContext.usDate || prevTradingDay(benchDate);

// consumedDate = Action 写入 est_nav_yesterday 时消费的指数日期
const consumedDate = base?.index_date;   // est_nav_index_date 字段

// 若两者相同 → 同一批指数已在 est_nav_yesterday 中用过，实时分量归零
const isUsDoubleCount = useChained && f.cat === 'us' && (
  consumedDate
    ? consumedDate === realtimeIndexDate          // 精确判断（有 est_nav_index_date）
    : !marketContext.usDate                       // 后向兼容（旧数据无此字段）
);
// isUsDoubleCount = true → calcDynamicNavReturn 时将所有 us* bench 分量置零
```

`prevTradingDay(dateStr)`：对 `_sortedTradingDates` 二分查找，返回严格早于 `dateStr` 的最近交易日。日历未加载时退化为前一日历日。
...
| `DEEPSEEK_API_KEY` | GitHub Secrets | DeepSeek API（QDII 持仓季报解析，需充值余额） |
...
sync_fund_data.py 主循环（每只 QDII，每日全量扫描）
  ...
  └─ _extract_holdings_from_pdf_deepseek()
        pdfplumber 提取前 25 页文本
        DeepSeek chat → JSON { holdings_date, holdings: [{name_en, name_zh, ratio}] }
...
`_ETF_NAME_TO_TQ`（`sync_fund_data.py:386`）：关键词匹配表，覆盖原油/黄金/白银/铜/能源/科技/印度等类别。

**添加新映射时注意顺序**：特殊词必须排在通用兜底词前（如 `'GSCI Crude Oil'` 必须在 `'S&P GSCI'` 前）。

### Q4 2025 持仓普查（2026-03-28）

首次对 15 只 QDII 基金进行全量普查（jjcc 无数据的境外基金），持仓数据来源为各基金 Q4 2025 季度报告。所有基金 `holdings_date = 2025-12-31`，已写入 `fund_daily.json`。

---

## QDII 持仓与 Bench 校准历史

### Q4 2025 普查发现的 Bench 偏差（2026-03-28 修正）

| 基金 | 旧 Bench | 新 Bench | 原因 |
|------|---------|---------|------|
| 501312 海外科技 | QQQ 80% + HSTECH 10% + A股 10% | QQQ 60% + XLK 20% + SMH 20% | Q4 2025 全仓美国 ETF（ARK系+科技），**无港股、无A股** |
| 160216 国泰商品 | SGOL/GLD/GLDM/USO/SLV/CPER/XOP | CPER 37% + GLD 23% + GLDM 20% + SGOL 20% | **原油已完全清仓**，转为纯铜+黄金组合 |
| 161116 黄金主题 | sh518880（A股黄金ETF） | GLD 29% + SGOL 28% + GLDM 22% + IAU 21% | 持仓为美国黄金 ETF，改用 GC=F 链路（+FX 修正）更精准 |
| 160719 嘉实黄金 | sh518880 | usGLD | 同上 |
| 161815 银华抗通胀 | 含 usCPER（铜）| 9 只 ETF 重归一化（去铜） | Q4 2025 无铜持仓 |
| 501018 南方原油 | USO 60% / BNO 40% | USO 69% / BNO 31% | WTI 系实际占比约 61%，Brent 约 27% |
| 160723 嘉实原油 | 纯 usUSO | USO 68% + BNO 32% | 实际约 30% Brent 原油持仓，补充 BNO 分量 |

**注**：161116 和 160719 从 sh518880 切换到 usGLD 系的核心原因：sh518880 是人民币计价黄金 ETF，其价格变动中含 A 股溢价与 CNY 金价因素；而持仓的美国黄金 ETF（GLD/SGOL/IAU）是 USD 计价，通过 GC=F + FX 修正路径估值更准确。
